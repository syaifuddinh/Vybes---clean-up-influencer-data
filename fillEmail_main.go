...
func getDataFromQuery(queryString string) ([]InfluencerInterface, error) {
    ...
    for rows.Next() {
        ...
        err := rows.Scan(&influencer.URI, &influencer.Email)
        if err != nil {
            return nil, err
        }
...
func main() {
	...
	queryString = "SELECT rawinfluencer.uri, rawinfluencer.email AS email FROM rawinfluencer JOIN ( SELECT uri FROM (SELECT uri, COUNT(id) AS userAmount FROM rawinfluencer GROUP BY uri ) AS influencerGroup WHERE userAmount > 1) AS duplicatedInfluencers ON duplicatedInfluencers.uri = rawinfluencer.uri WHERE email IS NOT NULL"

	influencers, isSuccess := getDataFromQuery(queryString) 
	for _, influencer := range influencers {
		uri = influencer.URI
		email = strings.ReplaceAll(influencer.Email, "'", "\"")
		tableName = "rawinfluencer"
		updateCommand = "UPDATE " + tableName + " SET email = '" + email + "' WHERE uri = '" + uri + "'"
