...
func executeQuery(queryString string) (bool) {
    ...
    defer db.Close()
    // Define the query to retrieve the URI and email columns from the influencerTable
...
func getDataFromQuery(queryString string) ([]InfluencerInterface, error) {
    ...
    for rows.Next() {
        ...
        err := rows.Scan(&influencer.URI, &influencer.Source)
        if err != nil {
            return nil, err
        }
...
func main() {
	...
	queryString = "SELECT rawinfluencer.uri, rawinfluencer.source FROM rawinfluencer JOIN ( SELECT uri, selectedUserId FROM (SELECT uri, COUNT(id) AS userAmount, MIN(id) AS selectedUserId FROM rawinfluencer GROUP BY uri ) AS influencerGroup WHERE userAmount > 1) AS duplicatedInfluencers ON duplicatedInfluencers.uri = rawinfluencer.uri AND selectedUserId = rawinfluencer.id"
	...
	for _, influencer := range influencers {
		...
		updateCommand = "DELETE FROM " + tableName + "  WHERE source != '" + source + "' AND uri = '" + uri + "'"
